...
for i = 1:100
    ...
    q_table = zeros(12, 4, 4);



    %disp(keys)

    [q_table, rewards] = q_learning(q_table, actions, initial_state, final_state, 500);

    %[q_table, rewards] = sarsa(q_table, actions, initial_state, final_state, 500, 0.5, 0.1, 1);

    %[q_table, rewards] = expected_sarsa(q_table, actions, initial_state, final_state, 500, 0.5, 0.1, 1);
...
function [q_table, rewards] = q_learning(q_table, actions, initial_state, final_state, episodes, alpha, epsilon, gamma)
    if nargin < 8
        gamma = 1;
    end
    if nargin < 7
    ...
    for ep_num = 1:episodes
        ...
        while ~isequal(state, final_state)
            ...
            q_table(state(1), state(2), action) = q_table(state(1), state(2), action) + alpha * (reward + gamma * max_q_next - q_table(state(1), state(2), action));
...
function [q_table, rewards] = sarsa(q_table, actions, initial_state, final_state, episodes, alpha, epsilon, gamma)
    ...
    for ep_num = 1:episodes
        ...
        while ~isequal(state, final_state)
            ...
            if rand() < epsilon
                ...
                [~, next_action] = max(q_table(next_state(1), next_state(2), :));
            ...
            q_table(state(1), state(2), action) = q_table(state(1), state(2), action) + alpha * (reward + gamma * q_table(next_state(1), next_state(2), next_action) - q_table(state(1), state(2), action));
...
function [q_table, rewards] = expected_sarsa(q_table, actions, initial_state, final_state, episodes, alpha, epsilon, gamma)
...
for ep_num = 1:episodes
    ...
    while ~isequal(state, final_state)
        ...
        [~, best_action] = max(q_table(next_state(1), next_state(2), :));
        ...
        expected_value = (1 - epsilon) * q_table(next_state(1), next_state(2), best_action);

        % (1 - epsilon) % chance we choose other 3 actions
        for action_new = 1:4
            if action_new ~= best_action
                %key=sprintf('%d %d %d',state(1), state(2), act);
                expected_value = expected_value + q_table(state(1), state(2), action_new) * epsilon;
            end
        end
        ...
        q_table(state(1), state(2), action) = q_table(state(1), state(2),action) + alpha * (reward + gamma * expected_value - q_table(state(1), state(2), action));
